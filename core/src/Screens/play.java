...
public class play extends ScreenAdapter implements Screen, Serializable {
    MyGdxGame game;
    TmxMapLoader loader;
    TiledMap map;
    ...
    public play(MyGdxGame game, Tank t1, Tank t2) {
        this.game = game;
        batch = game.getBatch();
        ...
        player1  = new Game_player(440, 500, 50, 50, main_tank1, world);
        player2 = new Game_player(1300, 500, 50, 50, main_tank2, world);
    }
...
//
        ...
        sp_missile_1.addListener(new ClickListener(){
            @Override
            public void clicked(InputEvent event, float x, float y) {
                missile_name = "special_missile";
            }
        });
        System.out.println(missile_name);
        // position - setting
    ...
    public void render(float delta) {
        ...
        if(is_victory){
            ...
            if(elapsed > 10){
                game.database.COINS += 400;         // at game end we will reward with 400 coins
                game.database.GEMS += 20;         // at game end we will reward with 20 GEMS
                game.setScreen(new Main_menu(game));
            }
        }
...
//            main_missile.missile.applyLinearImpulse(impulse, new Vector2(main_player.position.x , main_player.position.y ), true);
            main_missile.missile.setLinearVelocity(impulse);
             end_time += delta;
